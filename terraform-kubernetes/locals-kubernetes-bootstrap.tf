locals {
  ...
  bootstrap_deployment_variable = {
    ...
    argocd_deployment_cas_node_annotator_helper_path       = "kubernetes/core/cas-node-annotator-helper"
    argocd_deployment_cas_node_annotator_helper_repository = local.platform_components.kubernetes.deployments.core.repository
    argocd_deployment_cas_node_annotator_helper_revision   = local.platform_components.kubernetes.deployments.core.revision
    argocd_deployment_cas_node_annotator_helper_self_heal  = local.platform_components.kubernetes.deployments.core.self_heal
...
EOT
    ...
    argocd_ingress = [
      for ingress_name, ingress in local.platform_components.kubernetes.ingresses : {
        ...
        platform_ingress_cloudflare_api_token               = try(ingress.integration, null) == "cloudflare" ? data.vault_generic_secret.cloudflare[ingress_name].data["api-key"] : "",
